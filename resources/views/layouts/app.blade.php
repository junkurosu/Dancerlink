...
@section('description'){{config('app.name')}}では地図を使ってレンタルダンススタジオを検索することが出来ます。ダンサーにとってスタジオは練習するのに欠かせない存在です。今までのサイトでは市区町村別にスタジオを探すことは出来ましたが見づらいものが多く探すのに時間がかかることがありました。そんな実体験からこのサイトは生まれました。ダンサーがスタジオ検索に時間をかけずにストレスなく使うことができるサイトになっています。@endsection
@else
@section('title')@yield('child-title') | {{config('app.name','Laravel')}}@endsection

@section('description')@yield('child-description'){{config('app.name')}}では地図を使ってレンタルダンススタジオを検索することが出来ます。ダンサーにとってスタジオは練習するのに欠かせない存在です。今までのサイトでは市区町村別にスタジオを探すことは出来ましたが見づらいものが多く探すのに時間がかかることがありました。そんな実体験からこのサイトは生まれました。ダンサーがスタジオ検索に時間をかけずにストレスなく使うことができるサイトになっています。@endsection
@endif
